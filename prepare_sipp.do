	...
	*Newest (2014) SIPP panel
	if `year'>=2013 {
		...
		*fix missing parental indicators for kids with partial years of data 
		cap frame drop parframe
		frame put ssuid epppnum rhcalmn tage_ehc, into(parframe)
		frlink m:1 ssuid epnmom rhcalmn, frame(parframe ssuid epppnum rhcalmn) 
		frget tage_ehc, from(parframe) prefix(pa_)
		...
		frlink m:1 ssuid epnmom rhcalmn, frame(parframe ssuid epppnum rhcalmn) gen(mom_link)
		frget tage_ehc, from(mom_link) prefix(mom_)
		compare tage_ehc mom_tage_ehc 
		replace epnmom = . if ch_mom==1 & tage_ehc >= mom_tage_ehc
		frlink m:1 ssuid epndad rhcalmn, frame(parframe ssuid epppnum rhcalmn) gen(dad_link) 
		frget tage_ehc, from(dad_link) prefix(dad_)
		compare tage_ehc dad_tage_ehc 
		replace epndad = . if ch_dad==1 & tage_ehc >= dad_tage_ehc

		drop ch_mom?* ch_dad?* parframe pa_tage_ehc mom_tage_ehc dad_tage_ehc mom_link dad_link
