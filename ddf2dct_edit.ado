...
program define ddf2dct_edit
 version 8.2
 syntax using/ [, dct(str) do(str) data(str) str(str) drop(str) keep(str) replace noinfile longstring sipptm]
  ...
  *D lines are var names, followed by var and value labels
  if substr(`"`line'"',1,2)=="D " {
   ...
   if "`sipptm'"!=""  local pos: word 5 of `line' //add special treatment for SIPP ddf files
   ...
   if "`:list v2-str'"!="" & !("`longstring'"!="" & `lngth'>12) {
    loc type //added special treatment for long variables--import as strings
